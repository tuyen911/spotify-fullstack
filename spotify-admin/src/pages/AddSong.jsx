...
const AddSong = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    if (!name || !desc || !song || !image || !duration) {
      toast.error('Please fill in all the information and upload both the image and song.');
      return;
    }

    setLoading(true);
    try {
      const selectedAlbum = album === 'none' ? '' : albumData.find(a => a._id === album)?.name || '';
      ...
      if (response.data.success) {
        ...
        // Safely reset file input fields
      ...
      console.error('Error occurred while adding the song:', error);
  ...
  return loading ? (
    ...
    <form onSubmit={onSubmitHandler} className="flex flex-col items-start gap-8 text-gray-600">
      ...
      <div className="flex flex-col gap-2.5">
        <p>Song Name</p>
        <input onChange={(e) => setName(e.target.value)} value={name} className="bg-transparent outline-green-600 border-2 border-gray-400 p-2.5 w-[max(40vw,250px)]" placeholder="Enter here" type="text" required />
      </div>
      <div className="flex flex-col gap-2.5">
        <p>Song Description</p>
        <input onChange={(e) => setDesc(e.target.value)} value={desc} className="bg-transparent outline-green-600 border-2 border-gray-400 p-2.5 w-[max(40vw,250px)]" placeholder="Enter here" type="text" required />
      </div>
      <div className="flex flex-col gap-2.5">
        <p>Album</p>
